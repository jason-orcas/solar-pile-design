...
def build_report_data() -> ReportData:
    ...
    return ReportData(
        project_name=st.session_state.get("project_name") or "Untitled",
        project_number=project_number or "",
        project_location=st.session_state.get("project_location") or "",
        project_notes=st.session_state.get("project_notes") or "",
        engineer_of_record=engineer_name or "",
        ...
        section=section,
        pile_embedment=_ss("pile_embedment", 10.0),
        above_grade=above_grade if above_grade is not None else 4.0,
        pile_type=st.session_state.get("pile_type") or "driven",
        head_condition=st.session_state.get("head_condition") or "free",
        bending_axis=st.session_state.get("bending_axis") or "strong",
        yield_strength=section.fy if section and section.fy else 50.0,
        ...
        design_method=st.session_state.get("design_method") or "LRFD",
        ...
        group_n_rows=int(_ss("group_n_rows", 1)),
        group_n_cols=int(_ss("group_n_cols", 1)),
        group_spacing=_ss("group_spacing", 36.0),
        group_x_spacing=_ss("group_x_spacing", 36.0),
        group_y_spacing=_ss("group_y_spacing", 36.0),
        group_piles=st.session_state.get("group_piles") or [],
        group_loads=st.session_state.get("group_loads") or [],
        group_head_condition=st.session_state.get("group_head_condition") or "Free",
        corrosion_enabled=bool(st.session_state.get("corrosion_enabled")),
        corrosion_design_life=_ss("corrosion_design_life", 0.0),
        corrosion_environment=st.session_state.get("corrosion_environment") or "",
        corrosion_coating=st.session_state.get("corrosion_coating") or "",
        corrosion_rate=_ss("corrosion_rate", 0.0),
        corrosion_t_loss=_ss("corrosion_t_loss", 0.0),
        ...
        topl_column_selected=st.session_state.get("topl_column_select") or "",
